...
def time_font(x): return ImageFont.truetype(
    '/opt/vilib/Roboto-Light-2.ttf', int(x / 320.0 * 6))


def text_font(x): return ImageFont.truetype(
    '/opt/vilib/Roboto-Light-2.ttf', int(x / 320.0 * 10))


def company_font(x): return ImageFont.truetype(
    '/opt/vilib/Roboto-Light-2.ttf', int(x / 320.0 * 8))
...
def add_text_to_image(name, text_1):
    ...
    time_size_x, time_size_y = image_draw.textsize(
        time_text, font=time_font(image_target.size[0]))
    text_size_x, text_size_y = image_draw.textsize(
        text_1, font=text_font(image_target.size[0]))
  ...
  # 设置文本文字位置
    ...
    company_xy = (
        text_size_y, image_target.size[1] - int(1.5*text_size_y) - text_size_y)

  # 设置文本颜色和透明度
    image_draw.text(time_xy, time_text, font=time_font(
        image_target.size[0]), fill=(255, 255, 255))
    image_draw.text(company_xy, text_1, font=text_font(
        image_target.size[0]), fill=(255, 255, 255))
...
# set parameters
    ...
    # 颜色的HSV空间中的 H 的范围
    color_dict = {'red': [0, 4], 'orange': [5, 18], 'yellow': [22, 37], 'green': [
        42, 85], 'blue': [92, 110], 'purple': [115, 165], 'red_2': [165, 180]}
...
# detect_obj_parameter
    ...
    # 标志文本 traffic_list = ['stop','right','left','forward'] 或 'none'
    ...
    # 手势文本  gesture_list = ["paper","scissor","rock"]
    ...
    detect_obj_parameter['picture_path'] = Default_Pictures_Path + \
...
# 返回检测到的交通标志的坐标，大小，类型，准确度
    ...
    def traffic_sign_detect_object(obj_parameter):
        if obj_parameter == 'x':
            ...
            # max_size_object_coordinate_y
            return -1*(int(Vilib.detect_obj_parameter['traffic_sign_y']/160.2)-1)
        elif obj_parameter == 'width':
            # objects_max_width
            return Vilib.detect_obj_parameter['traffic_sign_w']
        elif obj_parameter == 'height':
            # objects_max_height
            return Vilib.detect_obj_parameter['traffic_sign_h']
        elif obj_parameter == 'number':
            # objects_count
            return Vilib.detect_obj_parameter['traffic_sign_n']
        elif obj_parameter == 'type':
            return Vilib.detect_obj_parameter['traffic_sign_t']  # objects_type
        elif obj_parameter == 'accuracy':
            # objects_type
            return Vilib.detect_obj_parameter['traffic_sign_acc']
        return 'none'
...
# 返回检测到的手势的坐标，大小，类型，准确度
    ...
    def gesture_detect_object(obj_parameter):
        if obj_parameter == 'x':
            ...
            # max_size_object_coordinate_y
            return -1*(int(Vilib.detect_obj_parameter['gesture_y']/160.2)-1)
        elif obj_parameter == 'width':
            return Vilib.detect_obj_parameter['gesture_w']  # objects_max_width
        elif obj_parameter == 'height':
            # objects_max_height
            return Vilib.detect_obj_parameter['gesture_h']
        elif obj_parameter == 'type':
    ...
    def detect_color_name(color_name):
        if color_name == 'close':
            ...
            Vilib.detect_obj_parameter['lower_color'] = np.array(
                [min(Vilib.color_dict[Vilib.detect_obj_parameter['color_default']]), 60, 60])
            Vilib.detect_obj_parameter['upper_color'] = np.array(
                [max(Vilib.color_dict[Vilib.detect_obj_parameter['color_default']]), 255, 255])
            ...
            while True:
                ...
                for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
                    ...
                    if Vilib.detect_obj_parameter['change_setting_flag'] == True:
                        ...
                        change_setting_cmd = "camera." + Vilib.detect_obj_parameter['change_setting_type'] + '=' + str(
                            Vilib.detect_obj_parameter['change_setting_val'])
                        print(change_setting_cmd)
                        exec(change_setting_cmd)

                        change_type_dict[Vilib.detect_obj_parameter['change_setting_type']
                                         ] = Vilib.detect_obj_parameter['change_setting_val']
                    if Vilib.detect_obj_parameter['content_num'] != 0:

                        for i in range(Vilib.detect_obj_parameter['content_num']):
                            exec(
                                "Vilib.detect_obj_parameter['process_si'] = Vilib.detect_obj_parameter['process_content_" + str(i+1) + "'" + "]")
                            cv2.putText(img, str(Vilib.detect_obj_parameter['process_si'][0]), Vilib.detect_obj_parameter['process_si'][1],
                                        cv2.FONT_HERSHEY_SIMPLEX, Vilib.detect_obj_parameter['process_si'][3], Vilib.detect_obj_parameter['process_si'][2], 2)
                    ...
                    if Vilib.detect_obj_parameter['setting_flag'] == True:
                        ...
                        if setting_type == "resolution":
                            ...
                            cv2.putText(img, 'resolution:' + str(Vilib.detect_obj_parameter['setting_resolution']), (
                                10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                        elif setting_type == "shutter_speed":
                            change_type_dict["shutter_speed"] = Vilib.detect_obj_parameter['change_setting_val']
                            cv2.putText(img, 'shutter_speed:' + str(Vilib.detect_obj_parameter['change_setting_val']), (
                                10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                        else:
                            cmd_text = "Vilib.detect_obj_parameter['setting_val'] = camera." + \
                                Camera_SETTING[Vilib.detect_obj_parameter['setting']]
                            # print('mennu:',Ras_Cam.detect_obj_parameter['setting_val'])
                            exec(cmd_text)
                            cv2.putText(img, setting_type + ': ' + str(Vilib.detect_obj_parameter['setting_val']), (
                                10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    ...
                    if last_e != 'none':
                        cv2.putText(img, str(last_e), (0, 15),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (204, 209, 72), 2)
...
# 添加水印的控制开关
    ...
    def get_picture(process_picture):
        ...
        Vilib.detect_obj_parameter['picture_path'] = Default_Pictures_Path + \
            datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '.jpg'
...
# 交通标志检测函数，传入值依此是摄像头读取到图像，交通标志的坐标，长宽
    ...
    def traffic_predict(input_img, x, y, w, h):
        ...
        # 调整为识别模型的要求的96x96的图像大小
        resize_img = cv2.resize(
            new_img, (96, 96), interpolation=cv2.INTER_LINEAR)
    ...
    # Perform the actual detection by running the model with the image as input
        ...
        interpreter_1.set_tensor(
            input_details_2[0]['index'], image_np_expanded)  # 放入图像到模型中
...
# 手势识别的流程和上面交通标志一致
    ...
    # Perform the actual detection by running the model with the image as input
        ...
        interpreter_2.set_tensor(
            input_details_2[0]['index'], image_np_expanded)
    ...
    def traffic_detect(img):
        ...
        if Vilib.detect_obj_parameter['ts_flag'] == True:
            ...
            open_img = cv2.morphologyEx(
                mask_all, cv2.MORPH_OPEN, Vilib.kernel_5, iterations=1)  # 开运算
                        ...
                        if acc_val >= 75:
                            if traffic_type == 1 or traffic_type == 2 or traffic_type == 3:
                                ...
                                if circles is not None:
                                    for i in circles[0, :]:
                                        ...
                                        traffic_sign_coor = (
                                            int(x+i[0]), int(y+i[1]))
                                        cv2.circle(
                                            img, traffic_sign_coor, i[2], (255, 0, 255), 2)
                                        cv2.putText(img, str(traffic_dict[traffic_type]) + ': ' + str(round(acc_val)), (int(
                                            x+i[0]-i[2]), int(y+i[1]-i[2])), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)  # 加减10是调整字符位置
                                        if w * h > max_area:
                                            max_area = w * h
                                            max_obj_x = x
                                            max_obj_y = y
                                ...
                                red_mask_1 = cv2.inRange(
                                    hsv[y:y+h, x:x+w], (0, 50, 20), (4, 255, 255))
                                red_mask_2 = cv2.inRange(
                                    hsv[y:y+h, x:x+w], (163, 50, 20), (180, 255, 255))
                                ...
                                open_img = cv2.morphologyEx(
                                    red_mask_all, cv2.MORPH_OPEN, Vilib.kernel_5, iterations=1)  # 开运算
                                open_img = cv2.dilate(
                                    open_img, Vilib.kernel_5, iterations=5)
                                ...
                                if contours_count >= 1:
                                    ...
                                    blue_contours = sorted(
                                        blue_contours, key=Vilib.cnt_area, reverse=True)

                                    epsilon = 0.025 * \
                                        cv2.arcLength(blue_contours[0], True)
                                ...
                                #     # 分析几何形状
                                    ...
                                    if corners >= 0:
                                        ...
                                        cv2.putText(img, str(traffic_dict[traffic_type]) + ': ' + str(round(
                                            acc_val)), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)  # 加减10是调整字符位置
                                        if w * h > max_area:
                                            max_area = w * h
                                            max_obj_x = x
                                            max_obj_y = y
    ...
    def gesture_recognition(img):
        if Vilib.detect_obj_parameter['gs_flag'] == True:
            ...
            dst = cv2.calcBackProject(
                [target_hsv], [0, 1], roi_hsv_hist, [0, 180, 0, 256], 1)
            ...
            # print(len(faces))
                ...
                if w >= 60 and h >= 60 and face_len == 0:
                    ...
                    if acc_val >= 75:
                        # print(x,y,w,h)
                        cv2.rectangle(img, (int(x-0.1*w), int(y-0.2*h)),
                                      (int(x+1.1*w), int(y+1.2*h)), (0, 125, 0), 2, cv2.LINE_AA)
                        cv2.rectangle(img, (0, 0), (125, 27),
                                      (204, 209, 72), -1, cv2.LINE_AA)
                        cv2.putText(img, ges_dict[ges_type]+': '+str(acc_val) + '%', (0, 17),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)  # (0,97,240)
...
# 人脸检测
    ...
    def human_detect_func(img):
        if Vilib.detect_obj_parameter['hdf_flag'] == True:
            resize_img = cv2.resize(
                img, (320, 240), interpolation=cv2.INTER_LINEAR)            # 2.从BGR转换到RAY
...
# 颜色识别
    ...
    def color_detect_func(img):
        ...
        if Vilib.detect_obj_parameter['cdf_flag'] == True:
            resize_img = cv2.resize(
                img, (160, 120), interpolation=cv2.INTER_LINEAR)
            # 2.从BGR转换到HSV
            hsv = cv2.cvtColor(resize_img, cv2.COLOR_BGR2HSV)
            color_type = Vilib.detect_obj_parameter['color_default']
            mask = cv2.inRange(hsv, np.array([min(Vilib.color_dict[color_type]), 60, 60]), np.array(
                [max(Vilib.color_dict[color_type]), 255, 255]))           # 3.inRange()：介于lower/upper之间的为白色，其余黑色
            if color_type == 'red':
                mask_2 = cv2.inRange(hsv, (167, 0, 0), (180, 255, 255))
                mask = cv2.bitwise_or(mask, mask_2)

            open_img = cv2.morphologyEx(
                mask, cv2.MORPH_OPEN, Vilib.kernel_5, iterations=1)  # 开运算
                ...
                for i in contours:  # 遍历所有的轮廓
                    ...
                    if w >= 8 and h >= 8:
                        ...
                        if object_area > max_area:
                            ...
                            Vilib.detect_obj_parameter['color_x'] = int(
                                x + w/2)
                            Vilib.detect_obj_parameter['color_y'] = int(
                                y + h/2)
...
# 开启摄像头
    ...
    def camera_start(vflip=False, hflip=False):
        ...
        Vilib.camera_thread = threading.Thread(
            target=Vilib.camera_clone, name="camera_satrt")
...
# objects detection
    ...
    def object_detect_fuc(img):
        if Vilib.detect_obj_parameter['odf_flag'] == True:
            ...
            img = detect_objects(
                image=img, model=objects_detection_model, labels=objects_detection_labels)
        return img
...
# image classification
    ...
    def image_classify_fuc(img):
        if Vilib.detect_obj_parameter['icf_flag'] == True:
            ...
            img = classify_image(
                image=img, model=image_classification_model, labels=image_classification_labels)
        return img
...
# gesture detection
    ...
    def hands_detect_fuc(img):
        if Vilib.detect_obj_parameter['gdf_flag'] == True:
            img, Vilib.detect_obj_parameter['hands_joints'] = Vilib.detect_hands.work(
                image=img)
        return img
...
# pose detection
    ...
    def pose_detect_fuc(img):
        if Vilib.detect_obj_parameter['pdf_flag'] == True:
            img, Vilib.detect_obj_parameter['body_joints'] = Vilib.pose_detect.work(
                image=img)
        return img
