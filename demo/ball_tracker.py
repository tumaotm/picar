    ...
    while True:
        ...
        if r < BALL_SIZE_MIN:
            ...
            if follow_mode == 0:
                ...
                pan_angle += delta_pan
                delta_tilt = int(float(CAMERA_Y_ANGLE) /
                                 SCREEN_HIGHT * delta_y)
...
def find_blob():
    ...
    circles = cv2.HoughCircles(
        red_hue_image, cv2.HOUGH_GRADIENT, 1, 120, 100, 20, 10, 0)
    ...
    # print("circles: %s"%circles)
        try:
            ...
            center = (int(round(circles[0][closest_ball][0])), int(
                round(circles[0][closest_ball][1])))
    ...
    if show_image_enable:
        ...
        cv2.namedWindow(
            "Detected red circles on the input image", cv2.WINDOW_AUTOSIZE)
